...
interface CustomerViewCalendarProps {
  ...
  calendarRef: React.RefObject<View | null>;
...
export default function PropertyCalendarScreen() {
  ...
  const [lockModal, setLockModal] = useState<LockModalState>({ visible: false, date: '' });
  ...
  const fetchData = async () => {
    if (!id) return;
    try {
      const [propertyRes, reservationsRes, lockedRes] = await Promise.all([
        ...
        supabase.from('locked_dates').select('id, property_id, date, reason').eq('property_id', id),
      ]);

      setProperty(propertyRes.data);
      setReservations(reservationsRes.data || []);
      setLockedDates(lockedRes.data || []);
    } catch (error) {
  ...
  const dateStatusMap = useMemo(() => {
    const map = new Map<string, { status: 'available' | 'booked' | 'completed' | 'past' | 'locked'; reservation?: Reservation; lockedDate?: LockedDate }>();
  ...
  const getDateStatus = (day: number): { status: 'available' | 'booked' | 'completed' | 'past' | 'locked'; reservation?: Reservation; lockedDate?: LockedDate } => {
  ...
  const captureAndShareCalendar = async () => {
    if (!calendarRef.current || !property) {
      if (isWeb) {
    ...
    try {
      ...
      if (isWeb) {
        ...
        canvas.toBlob(async (blob) => {
          if (blob) {
            try {
              ...
              link.download = `calendar-${property.name}.png`;
              link.click();
        ...
        await Sharing.shareAsync(uri, {
          mimeType: 'image/png',
          dialogTitle: `Share ${property.name} Calendar`,
        });
      }
    } catch (error: any) {
  ...
  const handleCopyLink = async () => {
    ...
    try {
      const token = await createShareToken(property.id);
      ...
      {isCustomerView && property ? (
        ...
        <ScrollView style={styles.container}>
          ...
          <Modal
            ...
            transparent
            animationType="fade"
            onRequestClose={() => setLockModal({ visible: false, date: '' })}
          >
              ...
              <View style={styles.modalContent}>
                ...
                <View style={styles.modalButtons}>
                  <Pressable
                    ...
                    onPress={() => setLockModal({ visible: false, date: '' })}
                  >
