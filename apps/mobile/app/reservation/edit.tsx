...
const WebDateInput = ({ value, onChange, style, hasError }: {
  value: Date;
...
const recordDepositIncome = async (reservation: Reservation, formData: {
  guest_name: string;
  check_in: Date;
  check_out: Date;
  ...
  const { error } = await supabase.from('cashflow_entries').insert({
    ...
    type: 'income',
    category: 'deposits',
    description: `Deposit received - ${formData.guest_name} (${checkIn} - ${checkOut})`,
    amount: parseFloat(formData.deposit_amount) || 0,
...
const recordReservationIncome = async (reservation: Reservation, formData: {
  ...
  const description = hasDeposit
    ? `Balance payment - ${formData.guest_name} (${checkIn} - ${checkOut})`
    : `Reservation payment - ${formData.guest_name} (${checkIn} - ${checkOut})`;
...
export default function EditReservationScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [{ data: propsData }, { data: resData, error }] = await Promise.all([
        ...
        if (resData) {
          ...
          setForm({
            property_id: resData.property_id,
            guest_name: resData.guest_name,
            guest_phone: resData.guest_phone || '',
            guest_email: resData.guest_email || '',
            guest_count: String(resData.guest_count),
            check_in: new Date(resData.check_in + 'T00:00:00'),
            check_out: new Date(resData.check_out + 'T00:00:00'),
            source: resData.source || 'direct',
            status: resData.status,
            notes: resData.notes || '',
            total_amount: String(resData.total_amount || 0),
            deposit_amount: String(resData.deposit_amount || 0),
            deposit_paid: resData.deposit_paid || false,
          });
        }
      } catch (error) {
        console.error('Error fetching reservation:', error);
        showNotification('Error', 'Failed to load reservation');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    if (!validate() || !reservation) return;
    ...
    try {
      ...
      // Record deposit income when deposit_paid transitions to true
      const wasDepositPaid = reservation.deposit_paid;
      const isNowDepositPaid = form.deposit_paid;
      if (isNowDepositPaid && !wasDepositPaid && hasDeposit) {
        try {
      ...
      if (isNowCompleted && !wasCompleted) {
        // If deposit wasn't paid yet, record it now
        if (hasDeposit && !isNowDepositPaid) {
          try {
            const depositResult = await recordDepositIncome(reservation, {
              guest_name: form.guest_name.trim(),
              check_in: form.check_in,
      ...
      if (incomeRecorded && hasDeposit && depositRecorded) {
  ...
  if (isLoading) {
    return (
      <View style={styles.wrapper}>
        <Stack.Screen options={{ title: 'Edit Reservation', headerBackTitle: 'Back' }} />
    ...
    <View style={styles.wrapper}>
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.buttonContainer}>
          <Button title="Save Changes" onPress={handleSubmit} loading={isSaving} fullWidth />
          <Button
            title="Cancel"
            variant="ghost"
            onPress={() => router.back()}
